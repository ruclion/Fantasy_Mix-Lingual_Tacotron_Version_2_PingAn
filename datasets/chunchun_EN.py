...
def build_from_path(hparams, speaker_num, lan_num, input_dir, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
	...
	futures = []
	index = 1
	with open(os.path.join(input_dir, 'LJSpeech10000.txt'), encoding='utf-8') as f:
		for line in f:
			text = line.strip()
			basename = 'chunchun_EN_' + str(index)
			wav_path = os.path.join(input_dir, 'wavs_22k', '{}.wav'.format(index))
...
def _process_utterance(mel_dir, linear_dir, wav_dir, index, wav_path, text, speaker_num, lan_num, hparams):
	...
	except Exception as e: #catch missing wav exception
		print('file {} present in csv metadata is not present in wav folder or good. skipping!'.format(
			wav_path))
		print(str(e))
		return None
